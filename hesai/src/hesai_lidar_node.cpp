...
int main(int argc, char** argv) {
    ...
    RCLCPP_INFO(node->get_logger(), "Starting Hesai LiDAR Driver Node - REAL ORIGINAL VERSION");
    
    try {
        RCLCPP_INFO(node->get_logger(), "About to create HesaiDriver - this is where segfault should occur");
        
        // Create driver components
        auto driver = std::make_shared<hesai_lidar_driver::HesaiDriver>(node);
        
        RCLCPP_INFO(node->get_logger(), "HesaiDriver created successfully - no segfault occurred!");
